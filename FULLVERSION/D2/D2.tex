...
companions, secondary infall and shadow-driven dynamics. 
\begin{highlight}
Ultimately we wish
...
\item {\bf Secondary infall of a low-mass molecular cloudlet}\label{subproj-infall}\\
  ...
  the outer disk is not. We must keep in mind that the secondary infall may
  not just simply {\em create} a new ``outer disk'', but collide onto
  a {\em previously existing} outer disk with a completely different angular
  momentum direction. This would lead to violent hydrodynamic effects,
  and might torque the rotation axis of the pre-existing disk initially to
  a third direction (neither parallel to the initial disk, nor to the
  angular momentum axis of the infalling material). A calculation of this
  process is challenging, and grid codes such as RAMSES and PLUTO may not
  be the most suitable. SPH codes or Arepo might be better suited. We 
  may try out some models if we have the time, but this may also be
  a task for the second phase of this RU.
...
\item {\bf Year 3:}\\
  ...
  \ref{subproj-spirals}, \ref{subproj-infall} and \ref{subproj-rossby}
