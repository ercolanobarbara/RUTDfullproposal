...
%%%%  Please edit
%
Barbara Ercolano: Dust entrainment in disk winds (C2)}
%
%
%%%%%
...
diagnostics to quantitatively measure the mass loss rate and profile in disk 
...
radiation-hydrodynamic calculations performed in B1 and the dust model
developed in this project (C2)\footexplainconnect.} 
...
\begin{figure}
  ...
  \caption{Image from \citet{2011MNRAS.411.1104O}: Synthetic images for
    ...
    the \caphighlight{first column} shows the image in the H band,
    the \caphighlight{second column} displays the K band and the
    \caphighlight{third column} displays the L band. The
    \caphighlight{fourth column} displays a red green blue
...
distribution in the wind is thus necessarily incorrect. \connect{Given the
central role played by dust grains for the chemistry (see project B2), our project will
couple dust evolution in the underlying disk to the wind entrainment
...
\begin{enumerate}
\item \connect{Build a dust model for photoevaporative winds to be used in
astrochemistry and radiative transfer calculations (see B2).}
...
evolution of dust grains in the disk in order to efficiently produce
...
content in TD winds, compared to full disks. \connect{These models are
also relevant to the study of dust trapping in pressure bumps,
performed in project C1}. In any case, a more
...
\item \connect{Two-dimensional dust evolution results/parameterisation from the
...
in the wind \citep[see e.g.,][]{2011MNRAS.411.1104O, 2016MNRAS.463.2725H, 
2016MNRAS.461..742H} and they thus affect the chemistry in the wind
...
models including dust evolution in the disk become available (see next
section).} 
...
current/future instrumentations. \connect{Our results may motivate
...
\connect{As the first new models which account for grain evolution in
  the underlying disk become available for a few selected cases, they
  will be implemented in the astrochemistry models of
  project B2.  In collaboration with the postdoc employed for project
  B2, we will then compare the resulting chemical models with those
  obtained in the previous year, which assumed a standard MRN
  distribution for the underlying disk. This will allow us to estimate
  the effects due to the   coupling of  grain evolution and grain
  entrainment in the wind on the chemical abundance in the wind and
  disk atmosphere.} This step is important to establish to what degree
of complexity this coupling should be performed in further
...
be in place. We will now be able to apply it to a wide parameter
space of disk winds, \connect{producing sets of dust models to be passed on to
project B2 for the optimised chemical calculations.}

\connect{At this stage we will also connect with project C1,
...
stages. \connect{In order to do that we will use grids of TD disks from model B1
with cavities of various sizes, where the streamline topologies are
different from those of full disks.}

If time allows, \connect{the student will collaborate with the postdoc employed
for project B1 to produce full hydrodynamical simulations of disk winds, where the dust
component in the disk and wind is treated as particles (e.g.\ Picogna
\& Kley 2016).} As we discussed in the previous sections of this
...
performed in the second funding period. \connect{These will also include
photoevaporating disks with gaps opened by giant planets
\citep[e.g.,][]{2013MNRAS.430.1392R, 2015MNRAS.454.2173R}  and other
types of asymmetries, for which the models
developed in projects D1 (PI Kley) and D2 (PI Dullemond) will provide the starting conditions.}
...
% (conference, visit to collaborators) for both student and Applicant. 
% During the course of the PhD 2
...
% London in 2017). 
% %Note that no travel request is made for the PI
...
% conference fee; 1000 \EUR{} $\times$ 2 (6000 over 3 years).
% \item International trip: 6 overnight stays, airfare, conference fee;
%   1500 \EUR{} $\times$ 2 (9000 over 3 years).
...
Models for two-dimensional dust evolution of the underlying disk will be provided
by the ERC-funded group of Dr Birnstiel. In the third year of the
Research Unit we will connect with project C1 (PI Birnstiel \& Dullemond) to use the dust
distributions in the underlying disks. The project will also feed back
to C2 providing the grain entrainment to establish the effects on dust
evolution in the disk (e.g. streaming instability).
...
Dishoeck. At the end of the first funding period, if time allows, or
in the second funding period, we plan to investigate the effects of
non-axisymmetric underlying disks, for which the models developed in
projects D1 (PI Kley) and D2 (PI Dullemond) will provide the initial conditions. } 
